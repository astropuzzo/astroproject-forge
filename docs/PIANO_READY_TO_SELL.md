...
> Stato 27 luglio 2026 — aggiornamenti: corretto il controllo release quando il feed di canale non è ancora pubblicato. Il client usa GitHub Releases come fallback; le beta non firmate aprono la pagina di download, mentre il download interno resta riservato a installer con manifest, SHA-256 e Authenticode validi. Aggiunti test di regressione, pubblicazione feed bloccata per artefatti non firmati e guida pratica Windows/macOS. Resta da acquistare e configurare il certificato pubblico del titolare.
...
**Avanzamento 27 luglio 2026 — P0.14:** eliminata la dipendenza rigida dal tag mobile `channel-beta`. In assenza del feed, l’app interroga le release GitHub, seleziona il SemVer compatibile con Stable/Beta e distingue l’aggiornamento disponibile dal guasto di rete. Un artefatto non firmato può soltanto aprire la pagina release; non entra mai nel downloader verificato. Lo script `publish-update-channel.ps1` rifiuta manifest `signed: false`, impedendo la pubblicazione accidentale di un canale insicuro. Testati feed firmato, fallback GitHub, versione corrente e rifiuto Authenticode.
